    ...
    <div ref={ref} popover="auto" className="edja-detail" aria-label="Catch details">
      ...
      {stats.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {stats.map((s) => (
            <span
              key={s}
              className="rounded-full bg-lake-50 px-3 py-1 text-sm font-medium text-lake-700"
            >
              {s}
            </span>
          ))}
      ...
      {report.bait && (
        <div className="mt-3 text-sm text-slate-600">
          <span className="text-slate-400">Bait / lure:</span> {report.bait}
        </div>
      )}
      ...
      {report.photo_urls && report.photo_urls.length > 0 && (
        <div className="mt-4 grid grid-cols-2 gap-2">
          {report.photo_urls.map((url, idx) => (
            <div
              key={url}
              className="group relative aspect-square overflow-hidden rounded-xl border border-lake-100 bg-lake-50"
            >
              ...
              <button
                ...
                className="absolute bottom-1.5 right-1.5 inline-flex items-center gap-1 rounded-lg bg-white/90 px-2 py-1 text-xs font-medium text-slate-600 shadow-sm transition-colors hover:text-lake-700"
              >
      ...
      {canManage && (
        <div className="mt-5 flex gap-3 border-t border-slate-100 pt-4">
          <button
            ...
            className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 py-2.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50"
          >
          ...
          <button
            ...
            className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-red-200 py-2.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-50"
          >
            <Trash size={16} />
            Delete
          </button>
